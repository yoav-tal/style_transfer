...
class VGG(object):
    ...
    def conv2d_relu(self, prev_layer, layer_idx, layer_name):
        ...
        Inputs:
            ...
            layer_idx: the string that is the name of the current layer.
        ...
        with tf.variable_scope(layer_name, reuse=tf.AUTO_REUSE) as scope:
            kernels = tf.constant(W)
            bias = tf.constant(b)
            conv_calc= tf.nn.conv2d(input=prev_layer, filter = kernels, strides=[1,1,1,1], padding='SAME')
            bias_calc = tf.nn.bias_add(conv_calc, bias)
            out = tf.nn.relu(bias_calc)
        ###############################
    ...
    def avgpool(self, prev_layer, layer_name):
        ...
        ###############################
        ## TO DO
        #out = tf.layers.max_pooling2d(inputs=prev_layer,pool_size=[2,2],strides=2, padding='same')
        with tf.variable_scope(layer_name):
            out = tf.nn.avg_pool(value=prev_layer, ksize=[1,2,2,1], strides=[1,2,2,1],padding='SAME')
        #(input= prev_layer, window_shape=(2,2),pooling_type='avg', padding='SAME')
        ###############################
        setattr(self, layer_name, out)
